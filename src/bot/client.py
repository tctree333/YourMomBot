...
class YourMomClient(discord.Client):
    req_counter = 0
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(API_ENDPOINT, json=body) as r:
                    ...
                    if r.status == 200:
                        ...
                        self.logger.warning(f"API respond with code {r.status}.")
                        return None
    ...
    def block(text, original):
        ...
        _yourmom = yourmom.lower().replace(" ", "")
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not message.author.bot:
            ...
            yourmomify_content = await self.post_api(content)
            if not self.block(yourmomify_content, content):
                self.logger.info(f"Yourmomified: {yourmomify_content}")
                await message.channel.send(
                    content=yourmomify_content, reference=message, mention_author=False
                )
