...
class YourMomModel:
    _your_mom_node = nltk.tree.Tree("NP", ["your mom"])
    ...
    def __enter__(self) -> YourMomModel:
        return self
    ...
    # public methods
    def yourmomify(self, text: str, log: bool = True) -> List[str]:
        """
        Creates your mom jokes from given text.
        :param text: Text to yourmomify.
        :param log: Default True. Whether or not to log.
        :return: List of yourmomified strings. Each string in the list corresponds to a sentence in the input text. If the input contains 2 sentences, then the output will be a list of 2 strings.
        """
        bests, best_scores = [], []
        if log:
        ...
        for sent in ann["sentences"]:
            ...
            # replace NP with 'your mom'
            yourmomified = self._replace_np_with_ym(tree)

            if len(yourmomified) == 0:
                continue

            # check if list is non-empty
            for i, yourmom_sent in enumerate(yourmomified):
                # correct grammatical mistakes
                yourmomified[i] = self._correct_grammar(yourmom_sent)

            # evaluate toxicity of augmented setence,
            # find the most toxic one
            scores = self._toxic_score(yourmomified, key="toxic")
            if log:
                logger.debug(f"Detoxify scores: {list(zip(yourmomified, scores))}")
            best = yourmomified[np.argmax(scores)]
    ...
    def warm_up(self) -> None:
        ...
        """
        self.yourmomify(WARM_UP_TEXT, log=False)

    async def async_yourmomify(
        self, text: str, log: bool = True
    ) -> Awaitable[List[str]]:
        """
        Executes yourmomify asynchronously.
        """
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, functools.partial(self.yourmomify, text, log=log)
        )
    ...
    def _is_basic_response(text: str) -> bool:
        ...
        """
        pattern = re.compile(r"[^a-zA-Z0-9]*your mom[^a-zA-Z0-9]*")
        return pattern.fullmatch(text.lower()) is not None

    @classmethod
    def _replace_np_with_ym(cls, tree: nltk.tree.Tree) -> List[str]:
        """
        Generates all possible yourmom-substitutions given a tree by replacing noun phrases (NP).
        :param tree: The nltk tree to process.
        :return: List of yourmomified strings.
        """
        ...
        for np_pos in np_positions:
            ...
            # replace np with 'your mom'
            copy[np_pos] = cls._your_mom_node
